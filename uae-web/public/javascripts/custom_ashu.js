...
$('.modal-toggle').on('click', function (e) {
  ...
  if (data_id != undefined) {
    axios({
      url: `https://jedi-tech.co/api/leaderships/${data_id}`,
      method: 'GET',
      headers: {
      ...
      let content = `<div class="_inner_modal">
        <div class="_left">
            <div class="_cards">
                <img src="https://jedi-tech.co/api/${data.Media.url}">
            </div>
        </div>
        <div class="_right">
            <h2>${data.Name}</h2>
...
let _getInsightRow = function (id, mode) {
  if (id != undefined) {
    axios({
      url: `https://jedi-tech.co/api/${mode}/${id}`,
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json;charset=UTF-8'
      ...
      let content = `<div class="_inner_modal">
        ...
        <div class="_right">
            <h2>${data.Title}</h2>
            <span>${data.UploadedOn.substring(0,10)}</span>
		
            ${data.Description ? data.Description : ""}
        </div>
      </div>`
      $("#insightsModal").html(content)
...
let _getPressRow = function (id, mode) {
  if (id != undefined) {
    axios({
      url: `https://jedi-tech.co/api/${mode}/${id}`,
      method: 'GET',
      ...
      UploadedOn = UploadedOn.toString().slice(0, 10)
...
$(document).ready(function () {
  ...
  axios({
    url: `https://jedi-tech.co/api/milestones?_sort=Year:asc`,
    method: 'GET',
    headers: {
      'Accept': 'application/json',
      'Content-Type': 'application/json;charset=UTF-8'
    ...
    for (let i = 0; i < data.length; i++) {
      $(`.slick-dots li:nth-child(${i + 1})`).html(`<button><span>${data[i].Year}</span></button>`);
    }
  }).catch(err => {
...
$('body').find('._lifeCycleEvent').eq(0).removeClass('_opacity60')

$(document).on('click', '._lifeCycleEvent', function () {
  let index = $(this).attr('data-index');
  $('._cycle_section').find('._lifeCycleEvent').addClass('_opacity60');
  $(this).removeClass('_opacity60')
  $('#_building_life_cycle .slider_new').slick('slickGoTo', index);
})

$(document).on('click', '.slick-dots li', function () {
  let indexnew = $(this).index();

  $('._cycle_section').find('._lifeCycleEvent').addClass('_opacity60');
  $('._cycle_section').find('._lifeCycleEvent[data-index="' + indexnew + '"]').removeClass('_opacity60');
})